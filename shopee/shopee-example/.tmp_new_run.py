def run(self, shopee_link: str = None) -> bool:
        ...
        if mail_link:
            ...
            # STEP 4: Kiểm tra Chrome activity + dialog container (max 6s từ lúc nhấn)
            ...
            while self.running and time.time() < dialog_deadline:
                activity_ok = False
                if MODULES_AVAILABLE and self.serial:
                    ...
                    if code == 0 and "org.chromium.chrome.browser.ChromeTabbedActivity" in out:
                        activity_ok = True
                if activity_ok and MODULES_AVAILABLE:
            ...
            if chrome_dialog_found:
                ...
                if not self._tap_ratio(0.606, 0.558):
                    self.running = False
                    return False
                if not self._sleep(2.0):
                    self.running = False
                    return False
                if not self._tap_ratio(0.504, 0.906):
                    self.running = False
                    return False
                ...
                self.logger.info("Chrome dialog container not found, skipping...")
